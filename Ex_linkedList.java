...
public class Ex_linkedList {
	public static void main(String args[]) {
		...
		for(;;) {
			...
			System.out.println("1. 삽입, 2. 삭제, 3. 역순으로 변경, 4. 출력, 5. 프로그램 종료");
			...
			if(input.equals("1")) {
				...
				if(input1.contentEquals("1")) {
					...
					ListNode pre = L.searchNode(input121);
					if(pre == null) {
						System.out.println("이전에 입력한 데이터가 아닙니다.");
					}
					else {
						L.insertMiddleNode(pre, input122);
					}
				}
				...
				if(input2.contentEquals("1")) {
					...
					ListNode pre = L.searchNode(input21);
					if(pre == null) {
						System.out.println("이전에 입력한 데이터가 아닙니다.");
					}
					else {
						L.deleteMiddleNode(pre);
					}
				}
				...
				System.out.println("리스트를 역순으로 변경하였습니다.");
				L.reverseList();
			}
